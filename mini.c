...
void insert() // declaration
{
    ...
    // fflush is used to flush the previous memory then start writing because there are buffer
...
void search()
{
    ...
    while (fread(&s, sizeof(s), 1, fp) > 0 && flag == 0)
    {
        ...
        {
            flag = 1;
            printf("\n\n\tSearch Successful And Student Records Is as follows");
            printf("\nRoll No\t\tName of Student\t\tSection\t\tMaths\t\tScience\t\tEnglish\t\tMarks\n");
            printf("%d\t\t\t%s  \t\t  %s \t\t%.2f \t\t%.2f \t\t%.2f \t\t%.2f\n\n\n", s.roll, s.name, s.sec,s.s1, s.s2, s.s3, s.marks);
        }
    }
    if (flag == 0)
    {
